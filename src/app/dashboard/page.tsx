...
export default function DashboardPage() {
  ...
  // Tip amounts stored as strings to allow natural typing
  ...
  // Editing state for inline tip updates (string-based editing)
  ...
  // Custom amount modal (now integrated into quick‑run row)
      ...
      <div className={`min-h-screen bg-[#FFFFFF] text-[#0B1E23] pb-36 font-sans antialiased transition-all duration-500 ${
        ...
        <div className="max-w-md mx-auto px-5 pt-6 space-y-6">
          ...
          <div className="flex justify-between items-center py-2">
            <div>
              <p className="text-[0.7rem] font-bold uppercase tracking-widest text-[#1A434E]">Tho Neo So Sanh Voi Chu Neo</p>
              <h1 className="text-2xl font-black text-[#0B1E23] tracking-tight">{techName || 'Independent Builder'}</h1>
            </div>
          ...
          <div className="grid grid-cols-1 gap-5">
            ...
            {/* Quick Price Selection Buttons (5 Columns) with Integrated Custom Amount */}
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="bg-[#1A434E] rounded-3xl p-5 text-white flex flex-col justify-between shadow-sm min-h-[160px] transition-transform active:scale-95 border border-[#E29A49]/10">
                ...
                <div>
                  ...
                  <div className="flex justify-between text-[10px] text-[#FFF0E2]/60 mt-2 pt-2 border-t border-white/10 font-mono">
                    <span>Total: ${dailyTotals.gross.toFixed(0)}</span>
              ...
              {/* Payouts Box */}
              <div className="bg-[#FFF0E2] rounded-3xl p-5 flex flex-col justify-between shadow-sm min-h-[160px] border border-[#1A434E] transition-transform active:scale-95">
                <div className="flex justify-between items-start">
            ...
            <div className="bg-[#1A434E] border-2 border-[#FFF0E2] rounded-3xl p-5 shadow-sm">
              <div className="flex justify-between items-center mb-3 px-1">
                <h2 className="text-xs font-bold uppercase tracking-wider text-[#ffff]">Today's Services Logged</h2>
              ...
              {transactions.length === 0 ? (
                <div className="text-center py-10 text-slate-400 text-xs italic">No items logged yet today.</div>
              ) : (
                <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                  {transactions.map((t) => {
                    ...
                    return (
                      <div key={t.id} className={`flex items-center justify-between bg-[#FFF0E2]/40 rounded-xl p-3 border border-[#FFF0E2] transition-all hover:bg-[#FFF0E2]/80 shadow-sm ${isDeleting ? 'opacity-30 translate-x-4' : ''}`}>
                        <div>
                          ...
                          <div className="relative flex items-center">
                            ...
                            {isEditing ? (
                              <input
                                ...
                                autoFocus
                                className="w-20 pl-7 pr-2 py-1 text-right text-xs bg-[#FFFFFF] font-mono font-bold text-[#0B1E23] rounded-lg border border-[#1A434E] focus:outline-none focus:ring-1 focus:ring-[#1A434E]"
                              />
                            ) : (
                              <button
                                onClick={() => setEditingTip({ id: t.id, value: t.tip_amount.toString() })}
                                className="w-20 pl-7 pr-2 py-1 text-right text-xs bg-[#FFF0E2]/60 hover:bg-[#FFF0E2] font-mono font-bold text-[#0B1E23] rounded-lg border border-[#E29A49]/20 transition-all cursor-text"
                              >
                                ${t.tip_amount.toFixed(2)}
                              </button>
                            )}
                          </div>
                          <button onClick={() => deleteTransaction(t.id)} className="text-red-400 hover:text-red-500 p-1.5 rounded-lg transition-all active:scale-90">
                            🗑️
                          </button>
                        </div>
                      </div>
            ...
            <div className="bg-[#FFFFFF] border-2 border-[#1A434E] rounded-3xl shadow-sm p-5 transition-all hover:border-[#E29A49]/20 hover:shadow-md">
        ...
        <div className="fixed inset-0 bg-[#0B1E23]/50 backdrop-blur-xs flex items-end justify-center z-50" onClick={() => !isSavingSettings && setShowSettings(false)}>
          <div className="bg-[#FFFFFF] rounded-t-[2rem] w-full max-w-md p-6 pb-10 space-y-5 shadow-2xl border-t border-[#FFF0E2] text-[#0B1E23] animate-slide-up" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="space-y-4 pt-1">
              ...
              <div className="space-y-1">
                ...
                <div className="flex gap-2">
                  <input
                    type="text"
                    value={tempButtonsString}
                    onChange={(e) => setTempButtonsString(e.target.value)}
                    onBlur={applyQuickButtonsString}
            ...
            <div className="flex gap-3 pt-4">
              <button onClick={() => setShowSettings(false)} disabled={isSavingSettings} className="flex-1 py-3 bg-slate-100 rounded-xl text-slate-500 font-bold text-xs hover:bg-slate-200 transition-all active:scale-95">
                Discard Changes
              </button>
