...
export default function DashboardPage() {
  ...
  const computeTotals = (items: { actual_price: number; tip_amount: number }[]) => {
    ...
    const commission = gross * (commissionPct / 100) // This calculates using the stored commissionPct
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (emp) {
        ...
        // CRITICAL FIX: Ensure we use the pulled database value here
  ...
  const recordCustomService = async () => {
    ...
    if (isNaN(amount) || amount <= 0) {
      showNotification('Please enter a valid amount', 'info')
      return
    }
    ...
    try {
      ...
      if (res.ok) {
        showNotification(`Saved custom $${amount} with a $${currentTip} tip!`, 'success')
        setTipAmount('')
        await fetchData()
        if (rangeStart && rangeEnd) await fetchRangeData(rangeStart, rangeEnd)
      } else {
  ...
  const updateTip = async (id: number, newTip: number) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        showNotification('Failed to update tip', 'error')
        await fetchData()
      }
    } catch (err) {
      await fetchData()
...
const saveSettings = async () => {
  ...
  return (
    <>
      ...
      <div
        className={`min-h-screen bg-[#FFFFFF] text-[#0B1E23] pb-36 font-sans antialiased transition-all duration-500 ${
      ...
      >
          ...
          <div className="grid grid-cols-1 gap-5">
            ...
            <div className="bg-[#FFF0E2] rounded-3xl p-5 border border-[#E29A49]/10 shadow-sm relative">
              <div className="flex justify-between items-center mb-2">
                <label className="block text-xs font-bold uppercase tracking-wider text-[#1A434E]">
                  Enter Tip Amount
                </label>
                <span className="text-[10px] font-bold bg-[#E29A49] px-2 py-0.5 rounded-md text-white">
                  Active
                </span>
              </div>
              <div className="relative">
                ...
                {Number(tipAmount) > 0 && (
                  <button 
                    ...
                    className="absolute right-4 top-1/2 -translate-y-1/2 text-xs text-red-500 hover:text-red-600 font-bold active:scale-90 transition-all"
                  >
                    Clear
                  </button>
                )}
            ...
            <div className="bg-[#1A434E] rounded-3xl shadow-md p-4 text-white relative overflow-hidden">
              ...
              <div className="grid grid-cols-5 gap-1.5">
                ...
                <button
                  ...
                  disabled={activePressBtn !== null}
                  className="py-2.5 rounded-xl bg-[#E29A49] hover:bg-[#E29A49]/90 text-white font-sans font-bold text-xs shadow-sm transition-all flex flex-col items-center justify-center col-span-5 mt-1.5 active:scale-[0.98]"
                >
                  {activePressBtn === 'custom' ? 'Saving Custom Amount...' : 'Enter Different Amount'}
                </button>
              </div>
            </div>
            ...
            <div className="bg-[#FFFFFF] border-2 border-[#FFF0E2] rounded-3xl p-5 shadow-sm">
              <div className="flex justify-between items-center mb-3 px-1">
                <h2 className="text-xs font-bold uppercase tracking-wider text-[#1A434E]">Today's Services Logged</h2>
                <span className="text-[10px] font-mono font-bold bg-[#FFF0E2] rounded-full px-2.5 py-0.5 text-[#E29A49] shadow-inner">
                ...
                <div className="space-y-2 max-h-64 overflow-y-auto pr-1">
                  {transactions.map((t) => {
                    ...
                    return (
                      <div
                        key={t.id}
                        className={`flex items-center justify-between bg-[#FFF0E2]/40 rounded-xl p-3 border border-[#FFF0E2] transition-all hover:bg-[#FFF0E2]/80 shadow-sm ${
                          isDeleting ? 'opacity-30 translate-x-4' : ''
                        }`}
                      >
                        <div>
                        ...
                        <div className="flex items-center gap-2.5">
                          <div className="relative flex items-center">
                            <span className="absolute left-2 text-[9px] font-bold text-[#1A434E]/60 uppercase">Tip</span>
                            <input
                              type="number"
                              step="0.01"
                              disabled={isUpdating}
                              ...
                              className="w-20 pl-7 pr-2 py-1 text-right text-xs bg-[#FFFFFF] font-mono font-bold text-[#0B1E23] rounded-lg border border-[#E29A49]/20 focus:outline-none focus:ring-1 focus:ring-[#1A434E]"
                            />
                          </div>
                          <button
                            onClick={() => deleteTransaction(t.id)}
                            className="text-red-400 hover:text-red-500 p-1.5 rounded-lg transition-all active:scale-90"
                            title="Delete Item"
                          >
                            🗑️
                          </button>
            ...
            <div className="bg-[#FFF0E2]/60 rounded-3xl border border-[#FFF0E2] shadow-sm p-5 space-y-4">
              ...
              <div className="bg-[#1A434E] rounded-2xl p-4 text-white flex flex-col justify-between shadow-md border border-[#E29A49]/10">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs border-t border-white/10 pt-2.5 text-center font-mono font-bold text-[#FFF0E2]/80">
                  <div>
                    <span className="text-[8px] text-[#FFF0E2]/60 uppercase font-bold block">Cash Portion</span>
                    <div className="text-white mt-0.5">${rangeTotals.cashAmount.toFixed(2)}</div>
                  </div>
                  <div>
                    <span className="text-[8px] text-[#FFF0E2]/60 uppercase font-bold block">Check Portion</span>
                    <div className="text-white mt-0.5">${rangeTotals.checkAmount.toFixed(2)}</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="bg-[#FFFFFF] border-2 border-[#FFF0E2] rounded-3xl shadow-sm p-5 transition-all hover:border-[#E29A49]/20 hover:shadow-md">
              <div className="flex justify-between items-center border-b border-[#FFF0E2] pb-2 mb-3">
                <span className="text-xs font-bold uppercase tracking-wider text-[#1A434E]">Weekly Totals</span>
                <span className="text-[10px] text-white font-mono font-bold bg-[#E29A49] px-2.5 py-0.5 rounded-full shadow-inner">
                  +${weeklyTotals.tips.toFixed(2)} Tips
                </span>
              </div>
              <div className="text-center mb-3">
                <span className="text-[9px] text-[#1A434E]/60 uppercase tracking-wide block">Total This Week</span>
                <div className="text-2xl font-mono font-black text-[#0B1E23]">${weeklyTotals.total.toFixed(2)}</div>
              </div>
              <div className="grid grid-cols-2 gap-2 text-center text-xs bg-[#FFF0E2]/50 p-2 rounded-xl font-mono font-bold text-[#0B1E23] shadow-inner">
                <div>
                  <p className="text-[8px] text-[#1A434E]/60 uppercase font-bold mb-0.5">Total Cash</p>
                  <p className="text-[#1A434E]">${weeklyTotals.cashAmount.toFixed(2)}</p>
                </div>
                <div>
                  <p className="text-[8px] text-[#1A434E]/60 uppercase font-bold mb-0.5">Total Check</p>
            ...
            <div className="bg-[#FFFFFF] border-2 border-[#FFF0E2] rounded-3xl shadow-sm p-5 transition-all hover:border-[#E29A49]/20 hover:shadow-md">
              <div className="flex justify-between items-center border-b border-[#FFF0E2] pb-2 mb-3">
                ...
                <span className="text-[10px] text-white font-mono font-bold bg-[#1A434E] px-2.5 py-0.5 rounded-full shadow-inner">
                  Split Rate: {commissionPct}%
                </span>
              </div>
              <div className="text-center mb-3">
                <span className="text-[9px] text-[#1A434E]/60 uppercase tracking-wide block">Total This Month</span>
                <div className="text-2xl font-mono font-black text-[#0B1E23]">${monthlyTotals.total.toFixed(2)}</div>
              </div>
              <div className="grid grid-cols-2 gap-2 text-center text-xs bg-[#FFF0E2]/50 p-2 rounded-xl font-mono font-bold text-[#0B1E23] shadow-inner">
                <div>
                  <p className="text-[8px] text-[#1A434E]/60 uppercase font-bold mb-0.5">Total Cash</p>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-40 flex justify-center px-4 pb-6">
          <div className="w-full max-w-md bg-[#1A434E] rounded-2xl shadow-xl px-6 py-3.5 flex justify-between items-center relative text-white border border-[#E29A49]/20">
            ...
            <button onClick={() => showNotification('Opened analytics view', 'info')} className="flex flex-col items-center gap-1 opacity-60 hover:opacity-100 transition-transform active:scale-90">
              <span className="text-xl">📊</span>
      ...
      {showCustomModal && (
        ...
        >
          ...
          >
            <div>
              <h3 className="text-base font-extrabold text-[#0B1E23] tracking-tight">Enter Custom Amount</h3>
              <p className="text-xs text-slate-400">Type in the price value below to save it.</p>
            </div>
            
            <div className="relative">
              <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-[#1A434E] font-bold font-mono">$</span>
              <input
                type="number"
                step="0.01"
                inputMode="decimal"
                ...
                className="w-full bg-[#FFF0E2]/40 rounded-xl pl-8 pr-3 py-3 text-base font-mono font-bold text-[#0B1E23] border border-[#E29A49]/20 focus:outline-none focus:ring-2 focus:ring-[#1A434E]/20"
                placeholder="0.00"
                autoFocus
              />
            </div>
            <div className="flex gap-2.5 pt-1">
              <button
                onClick={() => setShowCustomModal(false)}
                className="flex-1 py-2.5 bg-slate-100 rounded-xl text-slate-500 font-bold text-xs hover:bg-slate-200 transition-all active:scale-95"
              >
                Cancel
              </button>
              <button
                onClick={recordCustomService}
        ...
        >
          ...
          >
            ...
            <div className="flex justify-between items-center">
              ...
              {settingsSuccess && (
                <div className="text-emerald-500 text-sm font-bold flex items-center gap-1.5 animate-pulse">
                  <span>✨</span> Synced
                </div>
              )}
            </div>
            ...
            <div className="flex gap-3 pt-4">
              <button
                ...
                className="flex-1 py-3 bg-slate-100 rounded-xl text-slate-500 font-bold text-xs hover:bg-slate-200 transition-all active:scale-95"
              >
                Discard Changes
              </button>
              <button
